...
$pageTitle = $pageTitle ?? 'Customer Form';
$pageEyebrow = $pageEyebrow ?? 'Customer management';
$formAction = $formAction ?? '/customers/store';
$customer = $customer ?? [];
$errors = $errors ?? [];

$field = static function (string $key, string $default = '') use ($customer): string {
                ...
                <div class="erp-card p-4 p-xl-5">
                    ...
                    <form method="post" action="<?php echo htmlspecialchars($formAction, ENT_QUOTES, 'UTF-8'); ?>" class="row g-4">
                        ...
                        <div class="col-12 d-flex justify-content-end gap-2">
                            <a href="<?php echo htmlspecialchars(app_url('/customers'), ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-light rounded-4 px-4">Cancel</a>
                            <button type="submit" class="btn btn-dark rounded-4 px-4">Save customer</button>
                        </div>
                    </form>
                </div>
            </div>
